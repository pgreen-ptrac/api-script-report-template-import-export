...
def handle_import(auth):
    json_data = handle_load_report_template_data()
    print("Loaded file")
    ...
    response = request_import_report_template(auth.base_url, auth.auth_headers, auth.tenant_id, json_data)    
...
def handle_export(auth):
    report_templates = request_list_report_templates(auth.base_url, auth.auth_headers, auth.tenant_id)
